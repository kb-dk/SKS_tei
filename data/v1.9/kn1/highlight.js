// ----------------------------------------------------------------------------
// Zoom Search Engine 6.0.sks (15/Jul/2008; v1.8 2013-01-10)
// Highlight & auto-scroll script (DOM version), modified for SKS-E by Karsten Kynde
//
...
var JumpToFirstOccurance = true; // SKS-E, see function hightlight()
...
function ZRetrieveQuery()
{
  ...
  hl= hl.replace( /%C3%A6/, "Ã¦" );
  hl= hl.replace( /%C3%B8/, "Ã¸" );
  hl= hl.replace( /%C3%A5/, "Ã¥" ); // æ, ø, å, to make up for an error in MSIE 8, SKS-E, 1.6 KK 2012-02-21
...
//        {
	        ...
	        if (y < 100)
	            window.scrollTo(0,150);   // go to top of page
	        else
	            window.scrollTo(0,y-150); // show space of 50 above
                                        // to allow for fixed bar in SKS-E, v1.4/1.6 2012-02-23
//        }
    }
}
...
{	
	...
	if (document.body)
	{
      ...
      JumpToFirstOccurance= !(window.location.hash.substring(1) || (window.location.search.search(/hash=/)>=0));
                                                              // SKS-E, v1.4/1.6 KK 2012-02-17
