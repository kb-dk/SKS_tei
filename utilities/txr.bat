...
set p_txt=p1 p2 p30 p48 p95 p247 p252 p254 p255 p259 p260 p261 p264 p265 p270 p271 p277 p283 p284 p305 p306 p307 p309 p315 p318 p319 p327 p339 p341 p345 p350 p364 p372 p378 p381 p385 p400 p421 p424 p431 p433 p434 p435 p440 p447 p469 p551
rem p592, ingen kommentarer
...
echo TEKSTREDEGQRELSER, txr.xml
for %%t in (%ts_txt% %uts_txt% %jp_txt% %bd_txt%) do (
if not exist v1.9\%%t (echo %%t dir not exist
exit /b)
echo %%t
if exist v1.8\%%t\txr.xml   call kn12tei\xsltproc -o v1.9\%%t\txr.xml   kn12tei\txr2tei.xsl v1.8\%%t\txr.xml
if exist v1.8\%%t\int_1.xml call kn12tei\xsltproc -o v1.9\%%t\int_1.xml kn12tei\txr2tei.xsl v1.8\%%t\int_1.xml
if exist v1.8\%%t\int_2.xml call kn12tei\xsltproc -o v1.9\%%t\int_2.xml kn12tei\txr2tei.xsl v1.8\%%t\int_2.xml
)
